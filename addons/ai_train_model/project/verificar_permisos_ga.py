...
def listar_cuentas_y_propiedades_disponibles(analytics_admin_client):
    """
    Lista todas las cuentas y propiedades GA4 a las que tiene acceso la cuenta de servicio.
    - Muestra las cuentas y propiedades encontradas con sus IDs.
    - Da instrucciones si no se encuentran cuentas o propiedades.
    """
    if not analytics_admin_client:
        print("ERROR: No se proporcionó un cliente de Analytics Admin.")
        return
    ...
    try:
        ...
        for account in accounts:
            ...
            account_path = account.name
            account_id = account_path.split('/')[-1]
            
            print(f"\nCUENTA: {account_name} (ID: {account_id})")
            
            # Listar las propiedades asociadas a esta cuenta
            try:
                properties_response = analytics_admin_client.list_properties({
                    "filter": f"parent:accounts/{account_id}"
                })
                properties = list(properties_response)

                if not properties:
                    print(f"  ↳ No se encontraron propiedades para esta cuenta.")
                    continue
                ...
                for property in properties:
                    ...
                    print(f"  ↳ PROPIEDAD: {property_name} (ID: {property_id})")

            except Exception as e:
                print(f"  ERROR al listar propiedades de la cuenta {account_id}: {str(e)}")
                print("  ↳ Verifica que la cuenta de servicio tiene los permisos adecuados.")
